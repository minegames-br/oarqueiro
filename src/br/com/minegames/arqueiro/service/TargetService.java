...
public class TargetService {
	...
	public void createGroundTarget(FacingDirection facing) {
		...
		blockManipulationUtil.createNewWool(configService.getWorld(), l.getBlockX(), l.getBlockY() - 1, l.getBlockZ(),
				DyeColor.WHITE);
	...
	private Integer getMaxMoves() {
		...
		maxMoves = Math.abs(area.getPointA().getY() - (area.getPointB().getY())) - 7;
	...
	public void destrobyFloatingBlockTarget(BlockTarget bTarget) {
		...
		Location l1 = null;
		Location l2 = null;
		if (configService.getArena().getFacing() == FacingDirection.EAST
				|| configService.getArena().getFacing() == FacingDirection.WEST) {
	...
	public void destroyGroundBlockTarget(BlockTarget bTarget) {
		...
		Location l1 = null;
		Location l2 = null;
		// os alvos são criados dependendo da direção que a arena foi
		// construída. X ou Z mudam no caso de ser EAST / NORT
		if (configService.getArena().getFacing() == FacingDirection.EAST
				|| configService.getArena().getFacing() == FacingDirection.WEST) {
	...
	public Target hasHittenTarget(Block hit) {
		...
		while (it.hasNext()) {
			...
			if (target instanceof BlockTarget) {
				...
				if (l1.getBlockX() == l2.getBlockX() && l1.getBlockY() == l2.getBlockY()
